(ns wipo.main
  "Kotodama WASM entrypoint for the Wipo clean-room actor (L4) — Clojure port.
  ...
  facts (`wipo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"wipo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "wipo-compat" :tier tier :entities entities} 200])
